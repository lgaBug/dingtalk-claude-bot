...
# 使用 Node.js 的 os.tmpdir() 获取与 proxy.ts 一致的临时目录
...
if [ "$STOPPED" = false ]; then
  ...
  # 查找匹配的 proxy 进程 (node --import tsx .../proxy.ts <processName>)
  PROXY_PIDS=$(ps aux | grep "proxy.ts" | grep "$PROCESS_NAME" | grep -v grep | awk '{print $2}')
  ...
  if [ -n "$PROXY_PIDS" ]; then
    ...
    # 检查是否还有残留
    REMAINING=$(ps aux | grep "proxy.ts" | grep "$PROCESS_NAME" | grep -v grep | awk '{print $2}')
    if [ -n "$REMAINING" ]; then
...
# 精确匹配由 proxy 启动的 Claude CLI 进程 (包含 --session-id 或 --resume 参数)
CLAUDE_PIDS=$(ps aux | grep "/claude\|bin/claude" | grep -E "session-id|resume" | grep -v grep | awk '{print $2}')
...
if [ -n "$CLAUDE_PIDS" ]; then
  ...
  # 检查是否还有残留，强制杀死
  REMAINING=$(ps aux | grep "/claude\|bin/claude" | grep -E "session-id|resume" | grep -v grep | awk '{print $2}')
  if [ -n "$REMAINING" ]; then
    echo "      部分进程未退出，发送 SIGKILL..."
    for pid in $REMAINING; do
      kill -SIGKILL "$pid" 2>/dev/null
